...
bool LogMerge::getNextRecord_string(std::string &str)
{
    ...
    const unsigned char bucketIndex = static_cast<unsigned char>(bucket);
    if(bucketIndex >= m_sources.size())
        return false;

    const LogRecord* record = m_sources[bucketIndex]->get_CurrentRecord();
    if(record == NULL)
        return false;

    str.assign(record->m_str, record->m_str_size);
    m_sources[bucketIndex]->pop( );
