...
## Modern plain-log runner

The original environment requires Python 3.6, TensorFlow 1.8, Keras 2.2.2,
CUDA 9, and dataset-specific JSON. For current systems and plain-text logs,
`elise_plain.py` provides the same core LSTM probability model plus arithmetic
coding pipeline:

```shell
sudo apt-get install python3-numpy

python3 elise_plain.py train -i Apache.log -m apache.model \
  --max-bytes 100000 --epochs 1
python3 elise_plain.py compress -i Apache.log -m apache.model \
  -o Apache.elise
python3 elise_plain.py decompress -i Apache.elise -m apache.model \
  -o Apache.restored.log
cmp Apache.log Apache.restored.log
```
