...
# Set the version from the selected include directory. The pkg-config result may
# describe a different system installation when CLP uses its bundled LibLZMA.
if(LibLZMA_INCLUDE_DIR AND EXISTS "${LibLZMA_INCLUDE_DIR}/lzma/version.h")
    file(
        STRINGS
        "${LibLZMA_INCLUDE_DIR}/lzma/version.h"
        liblzma_VERSION_DEFINES
        REGEX "^#define LZMA_VERSION_(MAJOR|MINOR|PATCH) [0-9]+$"
    )
    foreach(liblzma_VERSION_PART MAJOR MINOR PATCH)
        string(
            REGEX MATCH
            "LZMA_VERSION_${liblzma_VERSION_PART} ([0-9]+)"
            liblzma_VERSION_MATCH
            "${liblzma_VERSION_DEFINES}"
        )
        set(liblzma_VERSION_${liblzma_VERSION_PART} "${CMAKE_MATCH_1}")
    endforeach()
    set(
        LibLZMA_VERSION
