...
from rottnest.backends.utils import (
    get_fs_from_file_path,
    get_physical_layout,
    get_metadata_and_populate_cache,
    get_result_from_index_result,
    return_full_result,
)
# refactor later 
...
def index_files_logcloud(file_paths: List[str], column_name: str, name = uuid.uuid4().hex, remote = None, 
    ...
    for i in tqdm(range(0, len(file_paths),batch_files)):
        ...
        if p.exitcode != 0:
            raise RuntimeError(
                f"LogCloud compression failed for group {num_groups} "
...
def search_index_logcloud(indices: List[str], query: str, K: int, columns = [], wavelet_tree = False, exact = False):
    ...
    if flag == 1:
        ...
        result = polars.from_arrow(result).filter(
            polars.col(column_name).str.contains(query, literal=True)
        )
        ...
        requested_columns = list(dict.fromkeys([column_name, *columns]))
        results = []
        start_time = time.time()
        for file_path in reversed_filenames:
            path = file_path.replace("s3://",'').replace("gs://",'').replace("gcs://",'')
            table = pq.read_table(
                path,
                filesystem=get_fs_from_file_path(file_path),
                columns=requested_columns,
            )
            df = polars.from_arrow(table)
            result = df.filter(polars.col(column_name).str.contains(query, literal=True))
            if len(result) > 0:
        ...
        if len(results) > 0:
            return polars.concat(results).head(K)
        else:
            return None
