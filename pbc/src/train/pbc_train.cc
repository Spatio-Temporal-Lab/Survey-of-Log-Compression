...
bool PBC_Train::CreateSecondaryEncoderData(char* pattern_buffer, int64_t& pattern_len) {
    switch (compress_method_) {
        ...
        case PBC_LZ4:
            return CreateLz4DictUsingCompressedData(pattern_buffer, pattern_len);
        case PBC_ONLY:
            // do nothig
            return true;
        default:
...
bool PBC_Train::CreateLz4DictUsingCompressedData(char* pattern_buffer, int64_t& pattern_len) {
    PBC::PBC_Compress* pbc_compress = new PBC_ONLY_Compress(symbol_size_, buffer_size_);
    pbc_compress->ReadData(pattern_buffer, pattern_len);
    ...
    char* each_input_data = new char[len_];
    char* compressed_data = new char[len_];
    char* dict_buffer = new char[DEFAULT_LZ4_DICT_SIZE];
    int dict_len = 0;
    ...
    do {
        ...
        size_t compress_result = pbc_compress->CompressUsingPattern(
            each_input_data, each_input_data_len, compressed_data);
        if (PBC::PBC_isError(compress_result)) {
            PBC_LOG(ERROR) << "Compress failed when CreateLz4DictUsingCompressedData."
                           << std::endl;
            delete pbc_compress;
            delete[] each_input_data;
            delete[] compressed_data;
        ...
        int copy_len = static_cast<int>(std::min<size_t>(compress_result, DEFAULT_LZ4_DICT_SIZE));
        if (copy_len >= DEFAULT_LZ4_DICT_SIZE) {
            pbc_memcpy(dict_buffer, compressed_data + compress_result - DEFAULT_LZ4_DICT_SIZE,
                       DEFAULT_LZ4_DICT_SIZE);
            dict_len = DEFAULT_LZ4_DICT_SIZE;
        } else {
            if (dict_len + copy_len > DEFAULT_LZ4_DICT_SIZE) {
                int keep_len = DEFAULT_LZ4_DICT_SIZE - copy_len;
                memmove(dict_buffer, dict_buffer + dict_len - keep_len, keep_len);
                dict_len = keep_len;
            }
            pbc_memcpy(dict_buffer + dict_len, compressed_data, copy_len);
            dict_len += copy_len;
        }
    } while (data_pos < len_);

    if (dict_len > 0) {
        pbc_memcpy(pattern_buffer + pattern_len, dict_buffer, dict_len);
        pattern_len += dict_len;
        pattern_buffer[pattern_len] = 0;
    }
