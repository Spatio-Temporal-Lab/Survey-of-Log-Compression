...
static bool ParseOptions(int argc, const char** argv) {
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(argv[i], "--help") || !strcmp(argv[i], "-h")) {
            ...
            } else if (!strcasecmp(argv[next_pos], "pbc_lz4")) {
                config.compress_method = PBC::CompressMethod::PBC_LZ4;
            }
...
static std::string CompressMethodToString(PBC::CompressMethod compress_method) {
    switch (compress_method) {
        ...
        case PBC::CompressMethod::PBC_ZSTD:
            return "PBC_ZSTD";
        case PBC::CompressMethod::PBC_LZ4:
            return "PBC_LZ4";
    }
...
static int PBCCompressFile() {
    ...
    if (config.outputfile_path) {
        output_fd = open(config.outputfile_path, O_WRONLY | O_TRUNC | O_CREAT, 0644);
        ...
        for (int i = 0; i < bytes_read; i++) {
            if (read_buffer[i] == '\n') {
                if (i == start_pos && last_read.empty()) {
                    if (!WriteAll(output_fd, "\n", 1)) {
                        PBC_LOG(ERROR) << "write failed" << std::endl;
                        return -1;
                    }
                    start_pos = i + 1;
                    continue;
                ...
                if (!PBC::PBC_isError(compressed_size)) {
                    if (!WriteAll(output_fd, compressed_buffer, compressed_size) ||
                        !WriteAll(output_fd, "\n", 1)) {
                        PBC_LOG(ERROR) << "write failed" << std::endl;
                        return -1;
                    }
    ...
    while ((bytes_read = read(input_fd, read_buffer, read_buffer_len)) > 0) {
        ...
        for (int i = 0; i < bytes_read; i++) {
            if (read_buffer[i] == '\n') {
                ...
                if (!PBC::PBC_isError(decompressed_size)) {
                    if (!WriteAll(output_fd, decompressed_buffer, decompressed_size) ||
                        !WriteAll(output_fd, "\n", 1)) {
                        PBC_LOG(ERROR) << "write failed" << std::endl;
                        return -1;
                    }
