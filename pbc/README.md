...
PBC can utilize other compression encoder to further compress the data that has already been compressed by PBC. Currently supported compression algorithms include FSE, FSST, ZSTD, and LZ4. Depending on the compression algorithm used, they are referred to as PBC_ONLY(only use pbc), PBC_FSE, PBC_FSST, PBC_ZSTD, and PBC_LZ4, respectively.
...
```bash
cd example
clang++ pbc_train_pattern.cc -L/usr/local/lib/pbc -I/usr/local/include -lpbc -lpbc_fse -lpbc_fsst -lzstd -llz4 -lhs -lpthread -o pbc_train_pattern
clang++ pbc_compress.cc -L/usr/local/lib/pbc -I/usr/local/include -lpbc -lpbc_fse -lpbc_fsst -lzstd -llz4 -lhs -lpthread -o pbc_compress
```
...
Usage: pbc [OPTIONS] [arg [arg ...]]
  ...
  --train-pattern -i <inputFile> -p <patternFile> [--compress-method <pbc_only/pbc_fse/pbc_fsst/pbc_zstd/pbc_lz4>] [--pattern-size <pattern_size>] [--train-data-number <train_data_number>] [--train-thread-num <train_thread_num>] [--varchar].
  --test-compress -i <inputFile> -p <patternFile> [--compress-method <pbc_only/pbc_fse/pbc_fsst/pbc_zstd/pbc_lz4>] [--varchar].
  ...
  --compress-method        Compress method, one of pbc_only, pbc_fse, pbc_fsst, pbc_zstd, pbc_lz4, default is pbc_only.
