...
# restore.py lives alongside this file; use an absolute path so the script
# can be invoked from any working directory.
_RESTORE_SCRIPT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "restore.py")
...
def procFiles(typename, fileBeginNo, fileEndNo, now_input, now_output, now_temp, type_template, suffix):
    ...
    for t in range(fileBeginNo, fileEndNo+1):
        order = "python3 " + _RESTORE_SCRIPT + " -I " + os.path.join(now_input,str(t)+suffix) + " -O " + os.path.join(now_output,str(t)+".col") + " -T " + type_template + " -t " + thread_temp + " -K " + kernel
...
if __name__ == "__main__":
    ...
    dataset = os.path.basename(input_path.rstrip("/"))
    #print("======================================> dataset:", dataset)


    now_input = input_path
    now_output = os.path.join(output_path, dataset) # ../../../out/restored/logshrink/Linux
    #print("======================================> now_input:", now_output)
