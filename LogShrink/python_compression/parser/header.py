...
class Header:
    ...
    def genFormat(self, head):
        ...
        if (pd.Series(Hpart_len).value_counts()[Hcount] < len(Hpart_len) * self.threshold): #Different lenght -> Regular string
            return 1, 0, "%s", [-1], []
        ...
        for t in HDic.keys():
            ...
            if not pivot.isdigit(): #String
                if (len(HDic) == 1): #Only one string
                    return  1, 0, "%s", [-1], []
                if (pd.Series(HDic[t]).value_counts()[pivot] < len(HDic[t]) * self.threshold): #Dynamic string -> Regular string
                    return  1, 0, "%s", [-1], []
                else:
                    Hformat += pivot
                    str_count += 1
                ...
                if (pd.Series(n_len).value_counts()[len_pivot] < len(HDic[t]) * self.threshold): # Variable length
                    num_length.append(-1)
                else: # Fix length
