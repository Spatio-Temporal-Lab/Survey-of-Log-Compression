...
int loadTemplate(string input, LengthSearch *templateSearch)
{
	...
	while (ifs.getline(line, LINE_LENGTH))
	{
		...
		{ // check delim line. a new template
			string p = m[1]; //捕获到的[]中的内容
			eid = atoi(p.c_str()); //string to int
			if (!start)
			{ // not the first line
				templateNode::sum++;
				if (!success)
...
//				 cout << "finish add" << endl;
		...
		else
		{
			...
			while (buf != NULL or return_value != 0) // 循环解析一个字符串中的所有token
			{
...
int main(int argc, char *argv[])
{
	...
	// 添加
	else printf("--------Head format open seccussed--------\n");
    ...
    // 添加
    printf("--------start loadTemplate--------\n");

    //出错位置，loadTemplate中的templateSearch->addTemplate中的LengthTemplatePool[TemplateLength].push_back(tempnode);
    //rnmd，修改g++版本到9.5就不报core dump了
