...
def property_mining(log_seqs=None, 
    ...
    # sampling = False
    # print("Step 3: Commonality and Variability Analyzing... ")
    t1 = time.time()
    if sampling:
        print("Step 3: Sequence sampling")
        if random_sam:
        ...
        print("Step 4: Commonality and Variability Analyzer... ")
        # t1 = time.time()
        # mined_property = {}
        # sampled_property = {"EH": [], "EE": [], "VV": []}
        tot_index = np.array([])
        for sid, idxs in sampled_seqs_index.items():
            ori_index = [np.arange(idx * h, min((idx + 1) * h, len(log_meta.eids))) for idx in idxs]
            if len(ori_index):
                ori_index = np.concatenate(ori_index)
            else:
                ori_index = np.array([], dtype=int)
        ...
        head_length = log_meta.headers.shape[1]
        ...
        assert head_length == log_meta.headers.shape[1]
    else:   
        print("Step 4: Commonality and Variability Analyzer w/o Sampling... ")
...
def pattern_matcher(col_var, pat, diff_r, dict_r, dictionary):
    ...
    for j, carr in enumerate(col_var):
        if j in pat:
            # What if not match ??
            suc, new_arr, all_pos = pattern_matcher_unit(pat[j], carr)
            if suc:
                params.append(new_arr)
                new_pat[j] = (new_arr.shape[0], all_pos, pat[j])
            else:
    ...
    for i, var in enumerate(col_var):
        ...
        if len(set(var)) > mt * len(var):
            ...
            if parsing is False:
                pat = ''
            if pat != '':
                suc, arr, all_pos = pattern_matcher_unit(pat, var)
                if suc:
                    arr_list.append(arr)
                    v_pat[i] = pat
                else:
...
def pattern_matcher_unit(pat, var):
    ...
    def split_arr(var_i, pat_i):
        ...
        return np.array(res), all_pos
        
    arr, all_pos = split_arr(var, pat)
    
    if arr is not None:
        arr = arr.T
        return True, arr, all_pos
    else:
