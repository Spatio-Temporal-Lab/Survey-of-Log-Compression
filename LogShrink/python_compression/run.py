...
def threadsToExecTasks(ds, files, now_input, now_output, now_temp, type_template):
    ...
    while curFileNumBegin < fileListLen:
        ...
        # 修改，注释掉，并直接调用 procFiles，而不是提交到线程池
        ...
        procFiles(ds, curFileNumBegin, curFileNumEnd, now_input, now_output, now_temp, type_template)
        curFileNumBegin = curFileNumEnd + 1
...
if __name__ == "__main__":
    ...
    log_file = "_".join(['E', str(encode_mode), 'C', str(column_mode),
                        'R', str(random_sam), 'P', str(parsing),
                         'V', str(re_mode), 'K', kernel, 'S', str(seq_sampling),
    ...
    if segmenting:
        ...
        while True:
            ...
            if count == blockSize:
                ...
                buffer = []
                cou += 1
        print("LOGSHRINK_LOAD_TIME: {:.6f}".format(time.time() - _t_load_start))
