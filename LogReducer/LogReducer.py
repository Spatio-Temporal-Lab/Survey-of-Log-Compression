...
def run_thulr(now_input, begin_no, end_no, out_temp, type_template):
    order = (
        "./THULR -I " + now_input
        + " -X " + str(begin_no)
        + " -Y " + str(end_no)
        + " -O " + out_temp
        + " -T " + util.path_pro(type_template)
        + " -E " + encoder_mode
        + " -D " + time_diff
        + " -F " + os.path.join(type_template, "head.format")
    )
    print(order + " " + threading.current_thread().name)
    return call(order, shell=True)

#return exec time (t2-t1)
def procFiles(typename, fileBeginNo, fileEndNo, now_input, now_output, now_temp, type_template):
    t1 = time.time()
    #parser (template matching + variable encoding via THULR)
    now_temp += threading.current_thread().name  + "/"
    if (not os.path.exists(now_temp)):
        os.mkdir(now_temp)
    _t_tmv_start = time.time()
    res = run_thulr(now_input, fileBeginNo, fileEndNo, now_temp, type_template)
    if (res != 0):
        ...
        # fallback: retry each block one-by-one to isolate bad segments
        for i in range(fileBeginNo, fileEndNo + 1):
            single_res = run_thulr(now_input, i, i, now_temp, type_template)
            if (single_res != 0):
                tempStr = "Error Occur at: {} thread: {}, fileNo: {} to {}".format(typename, threading.current_thread().name, i, i)
                print (tempStr)
                writeLog(str(output_path) + "Log_{}".format(datetime.date.today()), tempStr,'WARNING')
                atomic_addErrnum(1)
    print("LOGREDUCER_SEG_TEMPLATE_VAR_TIME: {:.6f}".format(time.time() - _t_tmv_start))
    #compress (7za LZMA generic compression)
    _t_gen_start = time.time()
    for i in range(fileBeginNo, fileEndNo + 1):
        filename = "{}".format(i)
        src_dir = now_temp + filename
        if (not os.path.exists(src_dir)):
            continue
        zip_path = str(now_output) + str(filename) + ".7z"
        compression_order = "7za a " + zip_path + " " + src_dir + "/*" + " -m0=LZMA"
        zip_res = call(compression_order, shell=True)
        if (zip_res != 0):
            tempStr = "Zip failed at: {} thread: {}, fileNo: {}".format(typename, threading.current_thread().name, i)
            print(tempStr)
            writeLog(str(output_path) + "Log_{}".format(datetime.date.today()), tempStr,'WARNING')
            atomic_addErrnum(1)
            continue
        call("rm -rf " + src_dir + "/*", shell=True)
    print("LOGREDUCER_SEG_GENERIC_TIME: {:.6f}".format(time.time() - _t_gen_start))
    t2 = time.time()
    ...
    if (mode == "Tot"):
        ...
        while True:
            ...
            if count == blockSize:
                ...
                buffer = []
                cou += 1
        print("LOGREDUCER_LOAD_TIME: {:.6f}".format(time.time() - _t_load_start))
    else:
        seg_path = input_path
