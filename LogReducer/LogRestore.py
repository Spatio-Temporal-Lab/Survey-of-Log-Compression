...
def procFiles(typename, file_ids, idx_begin, idx_end, now_input, now_output, now_temp, type_template):
    t1 = time.time()
    #parser
    ...
    for idx in range(idx_begin, idx_end + 1):
        block_id = file_ids[idx]
        order = "python3 ./restore.py -I " + os.path.join(now_input, str(block_id) + ".7z") + " -O " + os.path.join(now_temp, str(block_id) + ".col") + " -T " + type_template + " -t " + thread_temp
        ...
        if (res != 0):
            ...
            fail_log = os.path.join(now_temp, "restore_failed_{}.log".format(block_id))
            with open(fail_log, "wb") as fw:
                fw.write(detail.stdout)
            tempStr = "Error Occur at: {} thread: {}, fileNo: {}".format(typename, threading.current_thread().name, block_id)
            print(tempStr)
            print("Detailed restore log: {}".format(fail_log))
            writeLog(str(output_path) + "Log_{}".format(datetime.date.today()), tempStr,'WARNING')
            atomic_addErrnum(1)
            continue
    t2 = time.time()
    tempStr = "thread:{}, type:{}, fileNo: {} to {} , cost time: {}".format(threading.current_thread().name, typename, file_ids[idx_begin], file_ids[idx_end], t2 - t1)
    print (tempStr)
    writeLog(str(output_path) + "Log_{}".format(datetime.date.today()), tempStr,'WARNING')
    return t2 - t1
...
def threadsToExecTasks(typename, file_ids, now_input, now_output, now_temp, type_template):
    fileListLen = len(file_ids)
    curFileNumBegin = 0
    curFileNumEnd = 0
    ...
    while curFileNumBegin < fileListLen:
        ...
        future = threadPool.submit(procFiles, typename, file_ids, curFileNumBegin, curFileNumEnd, now_input, now_output, now_temp, type_template)
...
if __name__ == "__main__":
    ...
    for f in os.listdir(input_path):
        ...
        stem = f.rsplit(".", 1)[0]
        if stem.isdigit():
            all_files.append(int(stem))
    all_files = sorted(all_files)

    type_template = template_path
    
    filename = output_path.split("/")[-1] # Linux.log
    ...
    now_type = filename.split(".")[0] # Linux
    print(len(all_files))
    if len(all_files) == 0:
        print("No valid .7z block files found. Quit")
        exit(1)
    ...
    if (mode == "Tot"):
        ...
        for block_id in all_files:
            now_path = os.path.join(now_temp, str(block_id)+".col")
