...
def restore_heads(path, tempalte_path, level_msg):
    ...
    HeadValue = getValue()
    if len(HeadValue) == 0:
        return [], [isMulti, headRegex]
    first_key = sorted(HeadValue.keys())[0]
    totLength = len(HeadValue[first_key])
    for key in list(HeadValue.keys()):
        cur = HeadValue[key]
        if len(cur) == totLength:
            continue
        if len(cur) < totLength:
            fill = cur[-1] if len(cur) > 0 else 0
            HeadValue[key] = cur + [fill] * (totLength - len(cur))
        else:
            HeadValue[key] = cur[:totLength]
    now_num = 0
    now_str = 0
    ...
    for i in range(0, headLength):
        ...
        while fidx < len(now_format):
            if (now_format[fidx] == '%' and now_format[fidx+1] == 'd'): #Num
                try:
                    hn = headNumber[now_num] if now_num < len(headNumber) else -1
                    if now_num in HeadValue:
                        Heads = [Heads[t] + pendding(HeadValue[now_num][t], hn) for t in range(0, totLength)]
                    else:
                        Heads = [Heads[t] + "" for t in range(0, totLength)]
                except Exception:
                    print("restore_heads fallback at now_num={}, tot_length={}, len(headNumber)={}".format(now_num, totLength, len(headNumber)))
                    Heads = [Heads[t] + "" for t in range(0, totLength)]
                now_num += 1
                fidx += 2
            else: #String start
...
#    lines = util.load_array(path, False)
    ...
    #print(len(lines))
    for line in lines:
        # Some datasets (e.g. Spark) may contain bytes that are not valid UTF-8
        # in failed-match payloads. Use a tolerant decode so restore does not
        # abort an entire block on UnicodeDecodeError.
...
if __name__ == "__main__":
    ...
    call("7z x " + input_path + " -o" + temp_path, shell=True)

    # If Eid.eid is absent the block had no matched entries (all lines went to
    # load_failed / match_failed).  Emit them directly so the merge step sees
    # the correct line count for this block and restore.py exits cleanly.
    eid_path = os.path.join(temp_path, "Eid.eid")
    if not os.path.exists(eid_path):
        fw = open(output_path, "wb")
        for fname in ["load_failed.log", "match_failed.log"]:
            fb = os.path.join(temp_path, fname)
            if os.path.exists(fb):
                chunk = open(fb, "rb").read()
    ...
    fw = open(output_path, "wb")
    recover_fallback_num = 0
    recover_exception_num = 0
    for eid in Eids:
        if (eid == -1):
            if load_idx < len(LoadFailed):
                fw.write((LoadFailed[load_idx] + '\n').encode())
            else:
                fw.write(b"\n")
                recover_fallback_num += 1
            load_idx += 1
            continue
        if (eid == 0):
            if match_idx < len(MatchFailed):
        ...
        try:
            if head_idx < len(Heads):
                ...
                recover_fallback_num += 1
            head_idx += 1
            template = Templates.get(eid)
            variables = Varibales.get(eid)
            if template is None or variables is None:
                fw.write((head + "\n").encode())
                recover_fallback_num += 1
                continue

            now_idx = template_idx.get(eid, 0)
            now_var = 0
            fs = head

            for s in template:
                if (s == "<*>"):
                    arr = variables.get(str(now_var), [])
                    if now_idx < len(arr):
                        fs += str(arr[now_idx])
                    else:
                        fs += ""
                    ...
                    fs += s
            fw.write((fs + '\n').encode())
            template_idx[eid] = now_idx + 1
        except Exception:
            fw.write(b"\n")
            recover_fallback_num += 1
            recover_exception_num += 1

    fw.close()
    print("Restore fallback substitutions #: {}".format(recover_fallback_num))
    print("Restore fallback exceptions #: {}".format(recover_exception_num))
    #Clean up
    #call("rm -rf " + temp_path + "*", shell=True)
