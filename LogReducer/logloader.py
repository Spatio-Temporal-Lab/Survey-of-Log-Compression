...
class LogLoader(object):
    def __init__(self, headLength, isMulti, headRegex, maxLength):
        ...
        self.splitregex = re.compile(r'(\s+|\|)') #匹配一个或多个空白字符 或 匹配竖线符号|

    def formalize_message(self, lines):
        def get_content(line): # 从输入行的指定“头部”之后截取并返回剩余的内容
            count = 0
            in_head = False
            for idx, i in enumerate(line):
                ...
                if not self.splitregex.search(i):
                    ...
                    in_head = False
                if (self.headLength + 1 == count): # 到达头部长度阈值，返回后面的内容
                    return line[idx:].strip()
            return line.strip() # 无头部，返回整行
            #print("{}: count -> {}".format(line, count))
        def get_head(line_seg, headers, delimer):
            head_count = 0
            for idx, se in enumerate(line_seg):
                if (head_count >= self.headLength): # headLength默认为4
                    break
                if (idx % 2 == 0):
        ...
        if (self.isMulti):
            ...
            for line in lines:
                line_seg = self.splitregex.split(line.strip()) # 去除行首尾的空格和换行符，并使用预定义的正则表达式分割
                if not line.strip():
                    fail_count += 1
                    continue
                get_head(line_seg, headers, header_delimer)
    ...
    def load_to_dataframe(self, log_filepath):
        """ Function to transform sample log file to dataframe
        """
