...
class LogParser:
    ...
    def parseUnityEntry(self, log_dataframe):
        ...
        """
        event, templates = self.parse(log_dataframe) #Get templates，包括Eid和模板字典
        #print(templates)
        # print("Tree is: ")
    ...
    def parse(self, log_dataframe):
        ...
        # 将一个日志模板（logClust.logTemplate）添加到前缀树（Prefix Tree，也叫Trie树）中。这个前缀树被用来组织和查找相似的日志模板
        ...
        for idx, line in log_dataframe.iterrows():
            ...
            logmessageL = self.splitregex.split(line['Content'].strip()) # self.splitregex = re.compile(r'(\s+|:|<\*>|,|=)')
            logmessageL = preprocess(logmessageL) # 数字或ipv4地址替换为<*>
            ...
            # Matched, Add the new log message to the existing cluster
            else:
                matchEid = matchCluster.Eid
                newTemplate = getTemplate(logmessageL, matchCluster.logTemplate) # 不同之处用<*>代替
                if ''.join(newTemplate) != ''.join(matchCluster.logTemplate): 
                    matchCluster.logTemplate = newTemplate
            
            logEvent.append(matchEid)

        print('Parsing step1 done. [Time taken: {!s}]. 生成Trie树'.format(datetime.now() - start_time))
        ...
        print('Parsing step2 done. [Time taken: {!s}]. 生成模板字典'.format(datetime.now() - start_time))
